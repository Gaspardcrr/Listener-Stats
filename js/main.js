...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    const globalLoadingOverlay = document.getElementById('globalLoadingOverlay'); // Assurez-vous d'avoir cet élément dans votre HTML (par exemple, dans index.html)
    ...
    // Function to determine the correct subdirectory for an artist
    function getArtistSubdirectory(artistName) {
        const cleanedName = cleanFileName(artistName);
        if (!cleanedName) return '_Other'; // Fallback for empty or invalid names

        const firstChar = cleanedName.charAt(0);
        // Test if the first character is an English alphabet letter (case-insensitive)
        if (/[a-zA-Z]/.test(firstChar)) {
            return firstChar.toUpperCase(); // e.g., 'A', 'B', 'Z'
        } else {
            return '_Other'; // For numbers, symbols, or other non-alpha characters.
                             // THIS MUST MATCH THE FOLDER NAME YOU CREATED (e.g., 'data/_Other/')
        }
    }
    ...
    async function loadArtistsProgressively() {
        ...
        // Show global loading indicator immediately
        if (globalLoadingOverlay) {
            globalLoadingOverlay.style.display = 'flex';
            console.log("Global loading overlay set to display: flex.");
        }
        ...
        if (cachedData && cachedTimestamp && (now - parseInt(cachedTimestamp) < ONE_DAY_IN_MS)) {
            try {
                ...
                if (qParam) {
                    ...
                    currentDisplayLimit = allArtists.length; // If search is persistent, display all matching results
                }
                renderArtistsTable(allArtists, searchInput.value || '');
                if (globalLoadingOverlay) {
                    globalLoadingOverlay.style.display = 'none'; // Hide global loader
                    console.log("Global loading overlay hidden (from cache load).");
                }
                return; // Exit, data loaded from cache
            } catch (e) {
        ...
        // 2. If no cache, expired cache, or parse error, start loading from CSVs
        ...
        // Ceci est la première phase de l'affichage progressif
        ...
        // Ceci affiche le tableau avec les noms et des placeholders "Chargement..."
        renderArtistsTable(allArtists, searchInput.value || '');
        ...
        _allArtistsLoadedPromise = new Promise(async (resolve) => {
            ...
            if (basicArtists.length === 0) {
                ...
                if (globalLoadingOverlay) {
                    globalLoadingOverlay.style.display = 'none'; // Hide global loader
                    console.log("Global loading overlay hidden (no basic artists).");
                }
                resolve();  
                return;
            }
            ...
            const detailPromises = basicArtists.map(async (artist) => {
                // Utiliser la fonction getArtistSubdirectory pour déterminer le bon sous-dossier
                const subdirectory = getArtistSubdirectory(artist.nom);
                const cleanedName = cleanFileName(artist.nom);
                // Construire le chemin du fichier avec le sous-dossier
                const dataFilePath = `./data/${subdirectory}/${cleanedName}.csv`; 
            ...
            // Re-render le tableau avec les données complètes une fois chargées (met à jour les "Chargement..." avec les vraies valeurs)
            renderArtistsTable(allArtists, searchInput.value || '');
            
            // Hide global loading indicator once ALL data is loaded and rendered
            if (globalLoadingOverlay) {
                globalLoadingOverlay.style.display = 'none';
                console.log("Global loading overlay hidden (after full background load).");
            }
